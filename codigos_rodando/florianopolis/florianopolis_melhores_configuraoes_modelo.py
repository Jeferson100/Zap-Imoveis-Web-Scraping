...
cidade_nome = os.getenv("LOCALIZAZAO_COMPLETA", "Florianopolis, Santa Catarina, Brasil")
...
if train_path.exists() and test_path.exists():
    ...
    if not arquivos:
        raise FileNotFoundError(f"Nenhum arquivo {padrao} em {PASTA_DADOS}")
    dfs = [pd.read_parquet(a) for a in arquivos]
    dados = pd.concat(dfs, ignore_index=True)
    logger.info(f"Carregados {len(arquivos)} fontes: {len(dados):,} linhas")

...
arquivo_mais_recente = max(PASTA_DADOS.glob(f'{cidade}_imoveis_limpo_chave_mao*.parquet'), key=lambda f: f.stem.split('_')[-1])

data_mais_recente = arquivo_mais_recente.stem.split('_')[-1]

output_file   = PASTA_DADOS / f'{cidade}_com_ind_local__chave_mao_{data_mais_recente}.parquet' 
